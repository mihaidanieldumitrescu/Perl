...
sub generatePageHeader{
	...
	$header.= "<p>Generated on ". strftime ("%Y.%m.%d",localtime). " </p>\n";
	$header .= "<ul style=\"margin-left:100px\">\n";
	return $header;
}
	...
	<div>
		...
		foreach my $page (@pages) {
			if ($page eq $currentPage){
				...
				if ($1 < 1000){
					$naviBar .= '
						<li><a class="active"> Page '.$1.'-'.($1+99).'</a></li>';
				} elsif ($1 == 1000) {
						$naviBar .= '<li><a class="menu">...</a></li></ul></div>
				...
				<li><a href="'. (($1 eq "1") ? "output_index.html" : $page).'">Page '.$1.'-'.($1+99).'</a></li>';}
					...
					$naviBar .= '<li><a class="menu">...</a></li></li></ul></div>
							<div class="hidden-menu"><ul>
										<li><a href="'. (($1 eq "1") ? "output_index.html" : $page).'">Page '.$1.'-'.($1+99).'</a></li>';
				} else {
					$naviBar .= '
...
sub main{
	...
	foreach my $dirs (@filelist){
		...
		if ( $dirs =~ /.*/){
				...
				print OUT "<li><a href=\"$dirs\\GalleryHTML\\output_index.html \">$dirs \(".countFiles("$dirs")."\)</a></li>\n";
				}
		}
